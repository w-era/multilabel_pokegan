...
num_gpu = 4
retrain = True
pretrained = './models/dc_gan_bcel2'
...
class Generator(nn.Module):
    def __init__(self,num_gpu):
        ...
        self.deconvolutions = nn.Sequential(
            nn.ConvTranspose2d(noise_size,feature_size*32,4,1,0,bias=False),
    ...
    def forward(self,noise):
        x = noise.view(noise.size(0),noise_size,1,1)
...
class Discriminator(nn.Module):
    def __init__(self,num_gpu):
        ...
        self.convolution = nn.Sequential(
            ...
            nn.Dropout(0.4),
            #4*4
            nn.Conv2d(feature_size*32,1,4,1,0,bias=False),
        )
...
if __name__=='__main__':
    ...
    device = torch.device("cuda:6" if (torch.cuda.is_available() and num_gpu > 0) else "cpu")
    ...
    if (device.type=='cuda') and (num_gpu>1):
        G = nn.DataParallel(G,(6,5,3,2),dim=0)
    if retrain:
    ...
    if (device.type=='cuda') and (num_gpu>1):
        D = nn.DataParallel(D,(6,5,3,2),dim=0)
    if retrain:
    ...
    def G_train(batch_size,D,G,G_optimizer,criterion):
        ...
        target = torch.ones(batch_size,1).to(device=device)[:,:,None,None]
    ...
    def D_train(batch_size,D,G,D_optimizer,criterion,real_images,real_labels):
        ...
        real_target = torch.ones(batch_size,1).to(device=device)[:,:,None,None]
        real_loss = criterion(real_scores,Variable(real_target)).to(device=device)
        noise = Variable(torch.randn(batch_size, noise_size)).to(device=device)[:,:,None]
        fake_images = G(noise)
        fake_scores = D(fake_images)
        fake_target = torch.zeros(batch_size,1).to(device=device)[:,:,None,None]
