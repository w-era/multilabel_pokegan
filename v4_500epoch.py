...
num_gpu = 4
retrain = True
pretrained = './models/dc_rgan_bcel16'
...
if __name__=='__main__':
    ...
    if (device.type=='cuda') and (num_gpu>1):
        G = nn.DataParallel(G,(0,1,2,3),dim=0)
    if retrain:
    ...
    if (device.type=='cuda') and (num_gpu>1):
        D = nn.DataParallel(D,(0,1,2,3),dim=0)
    if retrain:
    ...
    plt.axis("off")
    ims = [[plt.imshow(np.transpose(i,(1,2,0)), animated=True)] for i in log['img_list']]
    ani = animation.ArtistAnimation(fig, ims, interval=50, repeat_delay=500, blit=True)
    plt.show()
    ani.save(filename=pretrained+'_fixed.gif', writer="pillow")
    ...
    plt.imshow(np.transpose(make_grid(real_batch[0].to(device)[:16], padding=5, normalize=True).cpu(),(1,2,0)))

    plt.subplot(1,2,2)
    plt.axis("off")
    plt.title("Fake Images")
    plt.imshow(np.transpose(log['img_list'][-1],(1,2,0)))
